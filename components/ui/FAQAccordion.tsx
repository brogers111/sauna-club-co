...
function FAQItem({ faq }: { faq: FAQ }) {
  ...
  return (
    <div className="border-b border-cream/10 py-4">
      ...
      >
        <h3 className="font-display text-xl uppercase tracking-wide text-cream">{faq.question}</h3>
        ...
        <span className={`inline-block shrink-0 transition-transform duration-300 ${open ? "rotate-90" : "rotate-0"}`}>
          <ArrowIcon direction="right" className="h-5 w-5 text-cream" />
        </span>
      </button>
      {open ? (
        <p className={`mt-3 text-cream/80 transition-opacity duration-300 ${answerVisible ? "opacity-100" : "opacity-0"}`}>
          {faq.answer}
        </p>
      ) : null}
    </div>
