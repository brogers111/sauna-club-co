...
export function ContactForm({ phone }: ContactFormProps) {
  ...
  if (submitted) {
    return (
      <p className="mt-8 text-center font-display text-2xl uppercase tracking-wide text-black">
    ...
    <>
      {phone ? (
        <p className="mt-3 text-center font-sans text-black/80">
          ...
          <a href={`tel:${phone}`} className="font-medium text-black hover:text-orange">
            {phone}
          </a>{" "}
      ...
      <form
        className="mt-8 flex flex-col gap-4 rounded-2xl border-2 border-orange bg-tan-light p-6 shadow-[8px_8px_0_0_var(--color-orange)] sm:p-8"
      ...
      >
        ...
        <div className="flex items-center justify-between gap-4">
          <span className="font-sans text-sm font-medium text-black">Are you a Sauna Club Co member?</span>
          <div className="inline-flex shrink-0 rounded-full border border-black/20 p-1">
            <button
              type="button"
              onClick={() => setIsMember(false)}
              aria-pressed={!isMember}
              className={`cursor-pointer rounded-full px-4 py-1 text-sm font-medium transition-colors ${
                !isMember ? "bg-orange text-tan-light" : "text-black"
              }`}
            >
              No
            </button>
